...
## 3. Operational image mirror — Kaggle LFW v4

Study 0/Study 1A already used:

- `jessicali9530/lfw-dataset/versions/4`

For Study 1B this remains an **operational image-byte mirror**, not the authority defining the View-1 identity partition. The 2026-08-27 preflight demonstrated that this package can materialise the image archive while omitting `peopleDevTrain.txt` / `peopleDevTest.txt`. Consequently the experiment now retrieves these two metadata files from the original UMass endpoint and verifies them against torchvision's published hashes.

## 4. Engineering lesson captured
...
This separation reduces dependency on a single hosting service and makes source replacement auditable without changing the scientific object.

## 5. Study 1B specific rule
